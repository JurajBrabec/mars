...
INSERT INTO `rlis` (`NAME`, `STORAGE`, `TYPE`, `PROTECTION_DP`, `PROTECTION_NBU`) VALUES
	...
	('10 Years Database Compliance Archive', 'Off-site', 'FS', 'years 10|weeks 52(0|1|2)', '10 years'),
	('10 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 10|weeks 52(0|1|2)', '10 years'),
	...
	('2 Years Database Compliance Archive', 'Off-site', 'ON', 'years 2|months 24|weeks 104', '2 years|24 months|5 years'),
	('2 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 2|months 24|weeks 104', '2 years|24 months|5 years'),
	('24 Months Database Off-site Copy', 'Off-site', 'ON', 'years 2|months 24|weeks 104', '24 months|2 years'),
	('24 Months Filesystem Off-site Copy', 'Off-site', 'FS', 'years 2|months 24|weeks 104', '24 months|2 years'),
	('3 Years Database Compliance Archive', 'Off-site', 'FS', 'years 3|months 36|weeks 156', '3 years|36 months|5 years'),
	('3 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 3|months 36|weeks 156', '3 years|36 months|5 years'),
	...
	('4 Years Database Compliance Archive', 'Off-site', 'FS', 'years 4|weeks 208', '4 years|5 years'),
	('4 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 4|weeks 208', '4 years|5 years'),
	('5 Years Database Compliance Archive', 'Off-site', 'ON', 'years 5|weeks 260', '5 years'),
	('5 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 5|weeks 260', '5 years'),
	...
	('6 Years Database Compliance Archive', 'Off-site', 'FS', 'years 6|weeks 312', '6 years|7 years'),
	('6 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 6|weeks 312', '6 years|7 years'),
	...
	('7 Years Database Compliance Archive', 'Off-site', 'ON', 'years 7|weeks 364', '7 years'),
	('7 Years Filesystem Compliance Archive', 'Off-site', 'FS', 'years 7|weeks 364', '7 years'),
...
if (table_exists('mars40')) then
	...
	insert into audit_nbu (
		...
		select
			...
			if(a.status='EXCEPTION',a.status,if(left(group_concat(distinct if(a.STORAGE='Off-site',IFNULL(n.slp_ret,n.vault_ret),n.schedule_ret)),64) regexp a.PROTECTION_NBU,'OK','WRONG')) as STATUS,
			a.COMMENT,
			count(distinct if(a.STORAGE='Off-site',IFNULL(n.slp_name,n.vault),n.policy)) as LISTS,
			'NBU' as SOURCE,
			...
			left(group_concat(distinct if(a.STORAGE='Off-site',IF(ISNULL(n.slp_name),IF(ISNULL(n.vault),NULL,'Vault'),'SLP'),n.schedule)),64) as RETENTION,
			left(group_concat(distinct if(a.STORAGE='Off-site',IFNULL(n.slp_name,n.vault),n.policy)),256) as LIST,
			left(group_concat(distinct if(a.STORAGE='Off-site',IFNULL(n.slp_ret,n.vault_ret),n.schedule_ret)),64) as PROTECTION
			from audit a
				left join mars40.nbu_audit n on (n.client regexp concat( '^',a.HOST_NAME,'(\.|$)'))
				where if(a.STORAGE='Off-site',IFNULL(n.slp_name,n.vault),n.policy) is not null
...
if (table_exists('mars30')) then 
	...
	insert into audit_dp (
		...
		select
			...
			left(group_concat(distinct if(a.STORAGE='Off-site',IF(ISNULL(m.copylist),NULL,'CopyList'),m.retention)),64) as RETENTION,
...
INSERT INTO comments (EXCEPTION,DATA_CENTER,DEVICE_ID,BACKUP_RLI_ID,COMMENT,OWNER) 
   ...
   ON DUPLICATE KEY UPDATE EXCEPTION=VALUES(EXCEPTION),COMMENT=VALUES(COMMENT),OWNER=VALUES(OWNER),UPDATED=NOW();
   DELETE FROM comments WHERE DATA_CENTER IN (SELECT DISTINCT TRIM(DATA_CENTER) FROM c) AND UPDATED<NOW()-INTERVAL 1 DAY;
DROP TABLE c;
END ;;
...
CREATE DEFINER=root@`%` EVENT event_audit
	ON SCHEDULE
		EVERY 1 DAY STARTS '2015-01-01 12:05:00'
...
CREATE DEFINER=`root`@`%` PROCEDURE `procedure_execute`(
...
#		date_format(now(),'%a %d. %b %Y'),date_format(now()-interval 5 second,'%H:%i'),
		'qrs_audit','QRS audit','Last month',recipient,'CSV',
...
elseif (table_exists('mars30')) then
	replace into mars30.config_scheduler (date,time,name,param1,param4,param6) values (
		date_format(now(),'%a %d. %b %Y'),date_format(date_add(now(),interval (15-minute(now())%15)*60-second(now()) second),'%H:%i'),
#		date_format(now(),'%a %d. %b %Y'),date_format(now()-interval 5 second,'%H:%i'),
		'QRS audit','q1|q2|q3',recipient,'CSV');
end if;
end ;;
