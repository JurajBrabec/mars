...
function processFiles( ) {
	...
	data = data.replace( /%NBU_BIN_HOME%/gim, config.Mandatory.NBU_BIN_HOME );
	data = data.replace( /%NBU_DATA_HOME%/gim, config.Mandatory.NBU_DATA_HOME );
	data = data.replace( /%PHP_HOME%/gim, config.Optional.PHP_HOME );
	data = data.replace( /%NBU2ESL_PATH%/gim, config.Optional.NBU2ESL_PATH );
	data = data.replace( /%NBU2SM9_PATH%/gim, config.Optional.NBU2SM9_PATH );
